...
class Unit(Piece, Actor):
    def __init__(self, name, team, position, initial_position=None, has_flag=False, jail_timer=0, q_values=None):
        """Unit piece, representing a controllable character on the board.
        """
        Piece.__init__(self, team, position, initial_position)
        Actor.__init__(self, 5, q_values if q_values is not None else {})
        self.name = name
